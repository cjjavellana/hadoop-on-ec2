...
sed -i "" -e 's/^role \:named_node.*$/role \:named_node, \%w\{master_node_ip\}/g' config/deploy/production.rb
sed -i "" -e 's/^role \:data_node.*$/role \:data_node, \%w\{data_node_ip\}/g' config/deploy/production.rb

# Update config/deploy/production.rb
master_ip=$(grep "hadoop-master-node" hdp.out | awk '{print $8}' | tr '\n' ' ' | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//') 
echo "Master Node IP: $master_ip"
sed -i "" -e "s/master_node_ip/${master_ip}/g" config/deploy/production.rb

# Find data nodes ip and convert from newline delimited to space delimited
data_nodes_ip=$(grep hadoop-data-node hdp.out | grep -v Creating | awk '{print $8}' | tr '\n' ' ' | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//' )
echo "Data Node IP: $data_nodes_ip"
sed -i "" -e "s/data_node_ip/${data_nodes_ip}/g" config/deploy/production.rb
